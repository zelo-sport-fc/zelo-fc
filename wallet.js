...
(function loadExternalLibraries() {
    ...
    if (!window.TonConnectUI && !document.getElementById('ton-connect-script')) {
        const scriptTon = document.createElement('script');
        scriptTon.id = 'ton-connect-script';
...
function initTonConnect() {
    try {
        const TONUI = window.TON_CONNECT_UI || window.TonConnectUI;
        if (TONUI && !tonConnectUI) {
            tonConnectUI = new TONUI.TonConnectUI({
...
async function renderWalletPage(container) {
    try {
        ...
        container.innerHTML = `<div style="color: #ff4d4d; padding: 20px; text-align: center;">Error loading wallet module. Please refresh.</div>`;
    }
}
...
window.claimCoinsToSolanaWallet = async function() {
    ...
    try {
        ...
        const response = await fetch(`${BACKEND_URL}/api/clai
